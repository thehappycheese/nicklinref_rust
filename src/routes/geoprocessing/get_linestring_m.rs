    ...
    let features = road_data.filter_map(|item|{
		...
		match query.f{
			...
			OutputFormat::LATLON=> Err(ErrorWithStaticMessage::new(
                "Invalid query type LATLON can only be used with the point query type. Please use f=JSON, or specify slk instead of slk_from and slk_to."
            )),
			OutputFormat::LATLONDIR=> Err(ErrorWithStaticMessage::new(
                "Invalid query type LATLONDIR can only be used with the point query type. Please use f=JSON, or specify slk instead of slk_from and slk_to."
            )),
		}
}